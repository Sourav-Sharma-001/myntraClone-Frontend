...
export default function PlaceOrderChild({ item, items, setItems }) {
  ...
  const itemUpdateQty = (qty, id) => {
    ...
    if (newQty > 0) {
      dispatch(updateQty({ qty: newQty, id }));
      setItemQty(newQty); 
    }
  };
          ...
          <div className="max-sm:ml-2">
            ...
            <div className="flex py-3 items-center">
              <div className="px-2 max-sm:flex max-sm:px-0">
                <label className="text-sm font-bold text-[#535766] bg-[#F5F5F6] px-3 py-1 max-sm:px-0" htmlFor="size">
                  Size:
                </label>
                <select
                  className="text-[14px] font-bold text-[#535766] bg-[#F5F5F6] px-3 py-1 max-sm:px-0"
                  name="size"
                  id="sizes"
              ...
              <div className="flex px-2 items-center">
                <label className="text-sm font-bold text-[#535766] bg-[#F5F5F6] px-3 py-1" htmlFor="size">
                  Qty:
                </label>
                <div className="text-[14px] font-bold text-green-600 bg-[#F5F5F6] mr-2 px-3 py-1">
                  {itemQty}
                </div>
                <div className="flex max-sm:grid max-sm:grid-cols-1">
                  <button
                    className="font-bold ml-2 text-[#535766] px-3 py-1 bg-[#F5F5F6] border-[2px] max-sm:ml-0"
                    onClick={() => itemUpdateQty(-1, item.id)}
                  >
                    -
                  </button>
                  <button
                    className="font-bold text-[#535766] px-3 py-1 bg-[#F5F5F6] border-[2px]"
                    onClick={() => itemUpdateQty(1, item.id)}
                  >
                    +
                  </button>
            ...
            <div className="flex items-center">
              ...
              <div className="text-sm text-[#535766]">
